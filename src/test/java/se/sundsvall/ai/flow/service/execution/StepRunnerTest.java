...
class StepRunnerTest {
	...
	void runStep_noExecutor() {
		...
		when(inputPreparation.prepare("2281", session, exec.getStep())).thenReturn(new InputCollector.Inputs(List.of(), "", List.of()));
		when(targetExecutorResolver.resolve(exec.getStep().getTarget().type())).thenThrow(new RuntimeException("No TargetExecutor"));

		final var ctx = new StepRunContext("2281", session, exec, List.of(), List.of(), "", null, true);
		final var result = runner.runStep(ctx);

		assertThat(result.success()).isFalse();
		assertThat(result.errorMessage()).contains("No TargetExecutor");

		verify(stepStateUpdater).markRunning(exec);
		verify(stepStateUpdater).markFailed(eq(exec), contains("No TargetExecutor"));
	}
	...
	void runStep_targetThrows() throws Exception {
		...
		when(targetExecutorResolver.resolve(exec.getStep().getTarget().type())).thenReturn(targetExecutor);
		...
		verify(stepStateUpdater).markRunning(exec);
		verify(stepStateUpdater).markFailed(eq(exec), contains("boom"));
	}

	@Test
	void runStep_requiredStepsInvoked() throws Exception {
		// Create a session with two steps where second requires first
		final var s1 = new Step().withId("S1").withName("S1").withOrder(1).withTarget(new Step.Target(Step.Target.Type.SERVICE, UUID.randomUUID()));
		final var s2 = new Step().withId("S2").withName("S2").withOrder(2).withTarget(new Step.Target(Step.Target.Type.SERVICE, UUID.randomUUID()));
		...
		when(inputPreparation.prepare("2281", session, exec2.getStep())).thenReturn(new InputCollector.Inputs(List.of(), "", List.of()));
		...
		final var ctx = new StepRunContext("2281", session, exec2, List.of(), List.of(), "", null, true);
		final var result = runner.runStep(ctx);

		assertThat(result.success()).isTrue();
		verify(requiredStepsRunner).ensureRequiredStepsExecuted(eq(exec2), isNull(), eq(true), any());
	}
}
